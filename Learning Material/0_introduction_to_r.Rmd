...
This dataset now contains 8 columns:
...
*BNF_SECTION*: This is a constant value of 'Antidepressant drugs', as these are the only drugs contained in this dataset.
*BNF_PARAGRPAH*: Antidepressant drugs can be subset into 4 smaller groups, defined by their BNF paragraph.
...
The BNF is structured hierarchically into Chapters, Sections, Paragraphs and Chemical Substances (Drugs).

For example:

- Amitriptyline hydrochloride is an actual antidepressant *DRUG*
- Amitriptyline hydrochloride is one of many *DRUG* within the 'Selective serotonin re-uptake inhibitors' *BNF_PARAGRAPH*.
- Selective serotonin re-uptake inhibitors is one of many *BNF_PARAGRAPH* within the 'Antidepressant drugs' *BNF_SECTION*
...
In summary, BNF chapters are split into sections, which are then split into paragraphs, and which are then split into actual drugs (i.e. a hierarchy).
